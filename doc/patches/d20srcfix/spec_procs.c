...
SPECIAL(buy_items)
{
  ...
  if (CMD_IS("buy")) {
    ...
    for (i = 0; i <= ((CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS) ? NUM_SW_WEAPON_TYPES : NUM_WEAPON_TYPES); i++) {
    ...
    if (!is_weapon) {
      for (i = 0; i < ((CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS) ? NUM_SW_SPEC_ARMOR_TYPES : NUM_SPEC_ARMOR_TYPES); i++) {
...
SPECIAL(crafting_station)
{
  ...
  if (!station->contains) {
    if (CMD_IS("augment"))
      send_to_char(ch, "You must place at least two crystals of the same type into the station in order to augment a crystal.\r\n");
    ...
    if (CMD_IS("create"))
      send_to_char(ch, "You must place an essence, an item to use as the mold pattern, a crystal and your crafting resource materials\r\nin the station and then type 'create <optional item description>'\r\n");
    if (CMD_IS("restring"))
      send_to_char(ch, "You must place the item to restring and enough essences to cover the item level in the crafting station.\r\n");
    else if (CMD_IS("resize"))
  ...
  for (o = station->contains; o != NULL; o = o->next_content) {
    if (CMD_IS("augment")) {
      if (o && GET_OBJ_TYPE(o) == ITEM_CRYSTAL) {
        if (crystal_type == 0)
          crystal_type = GET_OBJ_VAL(o, 0);
        if (GET_OBJ_VAL(o, 0) != crystal_type)
          continue;
        if (GET_OBJ_LEVEL(o) > crystal_level) {
          crystal_bonus += MAX(0, crystal_level / 4);
          crystal_level = GET_OBJ_LEVEL(o); 
          orig = o;
        }
        else {
          crystal_bonus += MAX(0, GET_OBJ_LEVEL(o) / 4);
        }
      }
      continue;
    }
    if (o && IS_POWER_ESSENCE(o)) {
      essence_one = o;
      bonus += GET_OBJ_VNUM(o) - 64099;
    }
    for (i = 0; i < NUM_CRAFT_TYPES; i++) {
      if (o && GET_OBJ_VNUM(o) == craft_pattern_vnums(i)) {
        orig = o;
        pattern_vnum = craft_pattern_vnums(i);
      }
    }    
    if (o && IS_POWER_ESSENCE(o)) {
      essence_one = o;
      bonus += GET_OBJ_VNUM(o) - 64099;
      add_bonus += GET_OBJ_VNUM(o) - 64099;
    }
    else if (o && GET_OBJ_TYPE(o) == ITEM_CRYSTAL) {
      crystal = o; 
      val = GET_OBJ_VAL(o, 0);
    }
  ...
  if (CMD_IS("augment")) {
    if (orig) {
      ...
      if (GET_OBJ_LEVEL(orig) <= crystal_level) {
        send_to_char(ch, "There is no possibility to augment any crystals in the station with that combination of items.\r\n");
        return TRUE;
      }
      sprintf(buf, "@wa crystal of@y %s@n max level@y %d@n", apply_types[GET_OBJ_VAL(orig, 0)], GET_OBJ_LEVEL(orig));
      orig->name = strdup(buf);
      orig->short_description = strdup(buf);
      sprintf(buf, "@wA crystal of@y %s@n max level@y %d@n lies here.", apply_types[GET_OBJ_VAL(orig, 0)], GET_OBJ_LEVEL(orig));
      orig->description = strdup(buf);
    }
    else {
      send_to_char(ch, "No crystal was detected in the crafting station to be augmented.");
      return TRUE;
    }
  }
  ...
  if (CMD_IS("supplyorder")) {
    ...
    switch(GET_AUTOCQUEST_MATERIAL(ch)) {
      ...
      case MATERIAL_STEEL:
        ...
        craft_type = SCMD_FORGE;
        obj_level = get_skill_value(ch, SKILL_BLACKSMITHING);
        break;
      case MATERIAL_LEATHER:
        ...
        craft_type = SCMD_TAN;
        obj_level = get_skill_value(ch, SKILL_TANNING);
        break;
      case MATERIAL_COPPER:
        if (!IS_PRECIOUS_METAL(material)) {
          send_to_char(ch, "You must place 2 units of %s or a similar type of material (all the same type) into the station to continue.\r\n", material_names[GET_AUTOCQUEST_MATERIAL(ch)]);
          return 1;
        }
        craft_type = SCMD_CRAFT;
        obj_level = get_skill_value(ch, SKILL_GOLDSMITHING);
        break;
      case MATERIAL_WOOD:
        if (!IS_WOOD(material)) {
          send_to_char(ch, "You must place 2 units of %s or a similar type of material (all the same type) into the station to continue.\r\n", material_names[GET_AUTOCQUEST_MATERIAL(ch)]);
        ...
        craft_type = SCMD_ASSEMBLE;
        obj_level = get_skill_value(ch, SKILL_WOODWORKING);
        break;
      case MATERIAL_HEMP:
        ...
        craft_type = SCMD_WEAVE;
        obj_level = get_skill_value(ch, SKILL_TAILORING);
        break;
      default:
        send_to_char(ch, "Invalid material type for supply order.  Please contact an admin with error code: SUP_ORD_001\r\n");
        return 1;
    }
  ...
  if (orig) {
    ...
    if (CMD_IS("augment") || CMD_IS("supplyorder"))
      GET_OBJ_LEVEL(created) = GET_OBJ_LEVEL(orig);
  }
  else {
    send_to_char(ch, "You must put a mold item in the crafting station first.\r\n"
                     "This is either a mundane item bought from the market wards in Palanthas, or a crafted item\r\n"
                     "created from such.\r\n");
    return 1;
  }

  if (!val && !CMD_IS("restring") && !CMD_IS("resize") && !CMD_IS("convert") && !CMD_IS("augment") && !CMD_IS("supplyorder")) {
    send_to_char(ch, "You have to use a proper crafting crystal in order to imbue this item with magical properties.\r\n");
    return 1;
  }
  ...
  if (CMD_IS("restring")) {
    if (bonus < GET_OBJ_LEVEL(created)) {
      send_to_char(ch, "You need to put more essences in the station to cover the item level.\r\n");
      return 1;
    }    
    if (get_skill_value(ch, SKILL_CRAFTING_THEORY) < GET_OBJ_LEVEL(created)) {
      send_to_char(ch, "Your crafting theory skill isn't high enough to restring this item.\r\n");
      return 1;
    }
  }
  ...
  if (GET_OBJ_TYPE(created) == ITEM_WEAPON) {
    if (weapon_list[GET_OBJ_VAL(created, 0)].material == MATERIAL_STEEL && !IS_HARD_METAL(material)) {
    ...
    while (created->affected[mn].modifier != 0 && mn < 6) {
      if (val == created->affected[mn].location) {
        if (bonus > created->affected[mn].modifier)
          ...
          send_to_char(ch, "This item already has this property, but you need to add more essences to make it more powerful.\r\n");
          return 1;
        }
      }
        mn++;
    ...
    if (mn > 5) {
      send_to_char(ch, "You cannot add additional properties to this object.  If attempting to enhance an existing property, you will need more powerful essences.\r\n");
      return 1;
    }
  ...
  GET_OBJ_COST(created) = 100 + GET_OBJ_LEVEL(created) * 50 * MAX(1, GET_OBJ_LEVEL(created) - 1) + GET_OBJ_COST(created);
    ...
    while (bonus != minbonus && GET_OBJ_LEVEL(created) > GET_OBJ_LEVEL(crystal)) {
    if (val == APPLY_AC_DEFLECTION || val == APPLY_AC_SHIELD || val == APPLY_AC_NATURAL || val == APPLY_AC_DEFLECTION || val == APPLY_AC_DODGE || val == APPLY_AC_ARMOR)
  ...
  while (GET_OBJ_LEVEL(created) >= CONFIG_LEVEL_CAP) {
    if (val == APPLY_AC_DEFLECTION || val == APPLY_AC_SHIELD || val == APPLY_AC_NATURAL || val == APPLY_AC_DEFLECTION || val == APPLY_AC_DODGE || val == APPLY_AC_ARMOR)
      created->affected[mn].modifier -= 10;
    else if (val == APPLY_HIT)
      created->affected[mn].modifier -= 5;
  ...
  if (HAS_FEAT(ch, FEAT_DWARVEN_CRAFTING)) {
    ...
    if (GET_OBJ_TYPE(created) == ITEM_ARMOR_SUIT || GET_OBJ_TYPE(created) == ITEM_ARMOR) {
      GET_OBJ_VAL(created, VAL_ARMOR_APPLYAC) += 10;
    }
  }
  ...
  rand = dice(1, 100);
  int x = 0;
  while (rand <= (1 + get_rp_bonus(ch, RP_CRAFT))) {
    x++;
    rand = dice(1, 100);
    if (GET_OBJ_TYPE(created) == ITEM_WEAPON) {
      created->affected[mn+1].location = APPLY_DAMAGE;
  ...
  int cost = GET_OBJ_COST(created) / 4;

  if (GET_GOLD(ch) < cost && GET_BANK_GOLD(ch) < cost && !CMD_IS("supplyorder")) {
    send_to_char(ch, "You need %d coins on hand or in the bank to make this item.\r\n", cost);
    return 1;
  }
  ...
  if (CMD_IS("checkcraft")) {
    ...
    int ticks = GET_ADMLEVEL(ch) ? 1 : (MAX(1, ((GET_OBJ_WEIGHT(created) / 10 * (HAS_FEAT(ch, FEAT_ELVEN_CRAFTING) ? 2 : 1)))));
    send_to_char(ch, "It will make use of your %s skill, which has a value of %d.\r\n", spell_info[skill].name, get_skill_value(ch, skill));
    send_to_char(ch, "This crafting session will take %d minutes and %d seconds.\r\n", ticks / 6, (ticks % 6) * 10);
    send_to_char(ch, "You need %d coins on hand or in the bank to make this item.\r\n", cost);
    if (bonus > created->affected[mn].modifier)
      send_to_char(ch, "You are wasting essences on this item.  Consider using the divide command to turn the essence into something smaller.\r\n");
    return 1;
  }

  if (CMD_IS("restring")) {
    send_to_char(ch, "You begin the alchemical process of transmuting your existing item into %s.\r\n", created->short_description);
  }
...
SPECIAL(bounty_contractor) {
  ...
  if (!strcmp(arg, "new")) {
    ...
    if (GET_OBJ_VNUM((struct obj_data *)me) == (CONFIG_CAMPAIGN == CAMPAIGN_DRAGONLANCE ? 2237 : (CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS ? 60100 : 60100)) && ch->bounty_gem > 0 && GET_CLASS_RANKS(ch, CLASS_BOUNTY_HUNTER) == 0) {
...
//    send_to_char(ch, "\r\nType companion <#> to get an animal companion.\r\n(You must have more druid and ranger(-4) ranks than the companion level to take). \r\n");
    ...
    if (CMD_IS("buy")) {
      ...
      if (((MAX(0, GET_CLASS_RANKS(ch, CLASS_RANGER))) + GET_CLASS_RANKS(ch, CLASS_DRUID)) < pet_list[mob_num].level) {
        send_to_char(ch, "Your total druid and ranger ranks must be equal to or greater than the level of the mob picked.\r\n");
        return 1;
      }
...
SPECIAL(crafting_quest) {
  ...
  switch (atype) {
    case ARTISAN_TYPE_SMITH:
      if (dice(1, 2) == 1) {
        if (dice(1, 2) == 1) {
          while (!IS_HARD_METAL(weapon_list[(itype = (dice(1, 1+MAX_WEAPON_TYPES)-1))].material)) {
            continue;
          }
          sprintf(desc, "%s", weapon_list[itype].name); 
        } else {
          while (!IS_HARD_METAL(armor_list[(itype = (dice(1, 1+NUM_SPEC_ARMOR_TYPES)-1))].material)) {
            continue;
          }          
        ...
        if ((roll = dice(1, 4)) == 1) {
          sprintf(desc, "a bracelet"); 
          GET_AUTOCQUEST_MATERIAL(ch) = MATERIAL_COPPER;
        } else if (roll == 2) {
          sprintf(desc, "a ring"); 
          GET_AUTOCQUEST_MATERIAL(ch) = MATERIAL_COPPER;
        } else if (roll == 3) {
          sprintf(desc, "a metal helmet"); 
          GET_AUTOCQUEST_MATERIAL(ch) = MATERIAL_STEEL;
        } else {
          sprintf(desc, "a medallion"); 
          GET_AUTOCQUEST_MATERIAL(ch) = MATERIAL_COPPER;
        }
      }
      break;
    case ARTISAN_TYPE_FARMER:
        if ((roll = dice(1, 5)) == 1) {
          sprintf(desc, "a full set of clothing"); 
        } else if (roll == 2) {
          sprintf(desc, "a suit of padded armor"); 
        } else if (roll == 3) {
          sprintf(desc, "a cloth hood"); 
        } else if (roll == 4) {
          sprintf(desc, "a pair of cloth gloves"); 
        } else {
          sprintf(desc, "a cloak"); 
        }
      GET_AUTOCQUEST_MATERIAL(ch) = MATERIAL_HEMP;
      break;
    case ARTISAN_TYPE_WOODSMAN:
      if (dice(1, 2) == 1) {
        if (dice(1, 2) == 1) {
          itype = dice(1, 1+MAX_WEAPON_TYPES)-1;
          while (!IS_LEATHER(weapon_list[itype].material) &&
                 !IS_WOOD(weapon_list[itype].material)) {            
            itype = dice(1, 1+MAX_WEAPON_TYPES)-1;
          }
          GET_AUTOCQUEST_MATERIAL(ch) = weapon_list[itype].material;
          sprintf(desc, "%s", weapon_list[itype].name); 
        } else {
          itype = dice(1, 1+NUM_SPEC_ARMOR_TYPES)-1;
          while (!IS_LEATHER(armor_list[itype].material) &&
                 !IS_WOOD(armor_list[itype].material)) {            
            itype = dice(1, 1+NUM_SPEC_ARMOR_TYPES)-1;
          }
        ...
        if ((roll = dice(1, 10)) == 1) {
          ...
          sprintf(desc, "a pair of gauntlets"); 
          GET_AUTOCQUEST_MATERIAL(ch) = MATERIAL_LEATHER;
        } else if (roll == 3) {
          sprintf(desc, "a wooden chest"); 
        } else if (roll == 4) {
          sprintf(desc, "a wooden table"); 
        } else if (roll == 5) {
          sprintf(desc, "a wooden chair"); 
        } else if (roll == 6) {
          sprintf(desc, "a wooden fishing rod"); 
        } else if (roll == 7) {
          sprintf(desc, "a wooden workbench"); 
        } else if (roll == 8) {
          sprintf(desc, "a leather helmet"); 
          GET_AUTOCQUEST_MATERIAL(ch) = MATERIAL_LEATHER;
        } else if (roll == 9) {
          sprintf(desc, "a leather bracer"); 
          GET_AUTOCQUEST_MATERIAL(ch) = MATERIAL_LEATHER;
        } else {
          sprintf(desc, "a leather belt"); 
          GET_AUTOCQUEST_MATERIAL(ch) = MATERIAL_LEATHER;
        }
      }
      break;
  ...
  send_to_char(ch, "You have been commissioned for a supply order to make %s.  We expect you to make %d before you can collect your reward.  Good luck.\r\n"
    ...
    if (GET_AUTOCQUEST_VNUM(ch) > 0 && GET_AUTOCQUEST_MAKENUM(ch) < 1) {
      ...
      send_to_char(ch, "You have completed your supply order contract for %s.\r\n"
                       "You receive %d quest points.\r\n"
                       "%d coins have been deposited into your bank account.\r\n"
                       "You receive %d experience points.\r\n", 
...
SPECIAL(player_shop)
{
  ...
  if (is_abbrev(arg, "buy")) {
    ...
    send_to_char(ch, "You have purchased %s for %d gold coins.\r\n", obj->short_description, cost);

    struct obj_data *money;

    money = create_money(cost);
    ...
    if (cost < 0) {
      send_to_char(ch, "You must sell it for at least 1 gold or set the price at zero to remove it from sale.\r\n");
      return 1;
    }
...
SPECIAL(emporium)
{
  ...
  if (!*arg) {
    send_to_char(ch, "The syntax for this command is: 'emporium buy <vnum> <bonustype> <bonus>' or 'emporium list <weapons|armor|other> <bonustype> <bonus>'.\r\n");
  ...
  if (is_abbrev(arg, "buy")) {
    if (!*arg2) {
      send_to_char(ch, "Please specify the vnum of the item you wish to buy.  You may obtain the vnum from the 'emporium list' command.\r\n");
    send_to_char(ch, "@YPlease note that bonuses of the same type @RDO NOT@Y stack in d20 rules.\r\n");
      return 1;
    }
    ...
    if (CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS && !((vnum >= 30200 && vnum <= 30261) || (vnum >= 30085 && vnum <= 30092) || vnum == 30095)) {
      send_to_char(ch, "That is not a valid vnum.  Please select again.\r\n");
      return 1;
    }

    if (CONFIG_CAMPAIGN != CAMPAIGN_STAR_WARS && !((vnum >= 30000 && vnum <= 30083) || (vnum >= 30085 && vnum <= 30092) || vnum == 30095 || (vnum >= 30100 && vnum <= 30105))) {
      send_to_char(ch, "That is not a valid vnum.  Please select again.\r\n");
      return 1;
    }
...
//    if ((bt == APPLY_AC_SHIELD || bt == APPLY_AC_DEFLECTION || bt == APPLY_AC_NATURAL || bt == APPLY_AC_ARMOR) && CONFIG_CAMPAIGN != CAMPAIGN_STAR_WARS)
//      obj->affected[0].modifier *= 10;
    ...
    if (GET_QUESTPOINTS(ch) < cost) {
      send_to_char(ch, "That item costs %d quest points and you only have %d.\r\n", cost, GET_QUESTPOINTS(ch));
    send_to_char(ch, "@YPlease note that bonuses of the same type @RDO NOT@Y stack in d20 rules.\r\n");
      return 1;
    }    
    ...
    send_to_char(ch, "You purchase %s for %d quest points.\r\n", obj->short_description, cost);
    return 1;
  }
  else if (is_abbrev(arg, "list")) {
...
//      obj->affected[0].modifier *= 10;
    ...
    send_to_char(ch, "The syntax for this command is: 'emporium buy <vnum>' or 'emporium list <weapons|armor|other>'.\r\n");
    send_to_char(ch, "@YPlease note that bonuses of the same type @RDO NOT@Y stack in d20 rules.\r\n");
    return 1;
  }
...
SPECIAL(lockbox)
{
  ...
  if (lb != (obj = get_obj_in_list_vis(ch, argument, NULL, world[IN_ROOM(ch)].contents))) {
    if (CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS)
      send_to_char(ch, "That is not a lockbox and cannot be sliced.\r\n");
    else
  ...
  if (roll < 15) {
    if (CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS) {
      sprintf(buf, "As you try to open the lockbox, the detonator activates and an explosion is heard, with black smoke escaping the seams of the lockbox.");
      act(buf, FALSE, ch, 0, 0, TO_CHAR);
      sprintf(buf, "As $n tries to open the lockbox, the detonator activates and an explosion is heard, with black smoke escaping the seams of the lockbox.");
...
SPECIAL(orphan)
{
  ...
  if (!*arg) {
    if (CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS)
    send_to_char(ch, "Who are you looking to send, and where to? (foster-home | sith-academy)\r\n");
  ...
  if (is_abbrev(arg2, "foster-home")) {
    ...
    if (CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS) {
      GET_ALIGN(ch)++;
      send_to_char(ch, "@WYou gain a light side point.@n\r\n");
    }
  } else if (CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS && is_abbrev(arg2, "sith-academy")) {
    sprintf(buf, "You send $p off to to the Sith Academy.");
    act(buf, FALSE, ch, lb, 0, TO_CHAR);
    sprintf(buf, "$n sends $p off to the Sith Academy.");
    act(buf, FALSE, ch, lb, 0, TO_ROOM);
    obj_from_room(lb);
    extract_obj(lb);
    lb = obj = NULL;
    orphans--;
    GET_ALIGN(ch)--;
    send_to_char(ch, "@RYou gain a dark side point.@n\r\n");
  ...
  if (!reward) {
    ...
    send_to_char(ch, "You are rewarded %d gold for your action.\r\n", reward);
  }

  send_to_char(ch, "Your quest points increase by %d for your action.\r\n", GET_LEVEL(ch));
...
//      obj_to_char(obj, mob);
      ...
      send_to_char(ch, "It cost you %d gold to have %s repaired.\r\n", cost, obj->short_description);    
    }
  } else {
  ...
  if (GET_GOLD(ch) < cost) {
    send_to_char(ch, "You don't have enough gold. You need %d and have %d.\r\n", cost, GET_GOLD(ch));
    return 1;
  }
