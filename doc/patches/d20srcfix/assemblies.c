    ...
    if (GET_CRAFTING_TICKS(ch) == 0) {
      ...
      if (TRUE || GET_CRAFTING_TYPE(ch) == SCMD_DIVIDE || GET_CRAFTING_TYPE(ch) == SCMD_FOREST || GET_CRAFTING_TYPE(ch) == SCMD_FARM ||
        ...
        if (GET_CRAFTING_TYPE(ch) == SCMD_DIVIDE) {
          sprintf(buf, "You harvest $p (x%d).  Success!!!", GET_CRAFTING_REPEAT(ch));
          act(buf, false, ch, GET_CRAFTING_OBJ(ch), 0, TO_CHAR);
          sprintf(buf, "$n harvests $p (x%d).", GET_CRAFTING_REPEAT(ch));
        ...
        if (GET_CRAFTING_TYPE(ch) == SCMD_MINE) {
          ...
          if (dice(1, GET_OBJ_LEVEL(GET_CRAFTING_OBJ(ch)) * 20) < skill_roll(ch, SKILL_MINING)) {
            obj2 = read_object(GET_OBJ_VNUM(GET_CRAFTING_OBJ(ch)), VIRTUAL);
            obj_to_char(obj2, ch);
            send_to_char(ch, "Your mining abilities have allowed you to mine twice the ore!\r\n");
          }
          if (dice(1, 100) < MAX(5, skill_roll(ch, SKILL_MINING) / 3)) {
            send_to_char(ch, "You find a crystal amongst the ore!\r\n");
            get_random_crystal(ch, MIN(30, get_skill_value(ch, SKILL_CRAFTING_THEORY)));
          }
          if (!ch->restringing)
            gain_artisan_exp(ch, exp);
        }
        /*if (GET_CRAFTING_TYPE(ch) == SCMD_HARVEST) {
          sprintf(buf, "You harvest $p.  Success!!!");
          act(buf, false, ch, GET_CRAFTING_OBJ(ch), 0, TO_CHAR);
          sprintf(buf, "$n harvest $p.");
          act(buf, false, ch, GET_CRAFTING_OBJ(ch), 0, TO_ROOM);
          exp = 30 + (GET_OBJ_LEVEL(GET_CRAFTING_OBJ(ch)) * MAX(1, ch->player_specials->crafting_exp_mult)) *
                (HAS_FEAT(ch, FEAT_ELVEN_CRAFTING) ? 2 : 1) * 2 * (get_skill_value(ch, SKILL_HARVESTING) / 5) *
                (GET_OBJ_MATERIAL(GET_CRAFTING_OBJ(ch)) == MATERIAL_MITHRIL ? 2 : 1);
          if (dice(1, GET_OBJ_LEVEL(GET_CRAFTING_OBJ(ch)) * 20) < skill_roll(ch, SKILL_HARVESTING)) {
            obj2 = read_object(GET_OBJ_VNUM(GET_CRAFTING_OBJ(ch)), VIRTUAL);
            obj_to_char(obj2, ch);
            send_to_char(ch, "Your harvesting abilities allowed you to attain double the harvest!\r\n");
          }
          if (dice(1, 100) < MAX(5, skill_roll(ch, SKILL_HARVESTING) / 3)) {
            send_to_char(ch, "Your harvesting abilities allowed you to find a crystal amongst the materials!\r\n");
            get_random_crystal(ch, MIN(30, get_skill_value(ch, SKILL_CRAFTING_THEORY)));
          }
          if (!ch->restringing)
            gain_artisan_exp(ch, exp);
          ...
          if (ch->craft_times > 0)
            ...
            sprintf(buf2, "@n");
          sprintf(buf, "You create $p%s.  Success!!!", *buf2 ? buf2 : "");
          act(buf, false, ch, GET_CRAFTING_OBJ(ch), 0, TO_CHAR);
          sprintf(buf, "$n creates $p%s.", *buf2 ? buf2 : "");
          act(buf, false, ch, GET_CRAFTING_OBJ(ch), 0, TO_ROOM);
...
//          gain_exp(ch, MAX(1, level_exp( GET_CLASS_LEVEL(ch), GET_REAL_RACE(ch)) / (500 + (GET_LEVEL(ch) * 20))));
          ...
          if (GET_OBJ_VNUM(GET_CRAFTING_OBJ(ch)) == GET_AUTOCQUEST_VNUM(ch)) {
            ...
            if (GET_AUTOCQUEST_MAKENUM(ch) == 0) {
              send_to_char(ch, "You have completed your supply order! Go turn it in for more exp, reputation and credits!.\r\n");
            } else {
      ...
      if (!PRF_FLAGGED(ch, PRF_CRAFTING_BRIEF)) {
        if (GET_CRAFTING_TYPE(ch) == SCMD_MINE)        
          ...
          send_to_char(ch, "You continue to harvest %s.\r\n", GET_CRAFTING_OBJ(ch)->short_description);
        }
