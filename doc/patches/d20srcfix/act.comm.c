...
ACMD(do_d20chat) {
  skip_spaces(&argument);
  if (!*argument) {
    send_to_char(ch, "What do you want to d20chat?\r\n");
    return;
  }
  if (strlen(argument) > 255) {
    send_to_char(ch, "Your communicated string cannot be longer than 255 characters.\r\n");
    return;
  }
  MYSQL *conn2 = NULL;
  int temp = CONFIG_CAMPAIGN;
  char query[MAX_STRING_LENGTH];
  if (CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS || CONFIG_CAMPAIGN == CAMPAIGN_DRAGONLANCE) {
    // Open mysql connection
    conn2 = mysql_init(NULL);
    /* Connect to database */
    temp = CONFIG_CAMPAIGN;
    ...
    char *end;
    end = stpcpy(query, "INSERT INTO d20chats (name,theme,message) VALUES(");
    *end++ = '\'';
    end += mysql_real_escape_string(conn2, end, GET_NAME(ch), (strlen(GET_NAME(ch))));
    *end++ = '\'';
    *end++ = ',';
    *end++ = '\'';
    end += mysql_real_escape_string(conn2, end, campaign_abbrevs[CONFIG_CAMPAIGN], (strlen(campaign_abbrevs[CONFIG_CAMPAIGN])));
    *end++ = '\'';
    *end++ = ',';
    *end++ = '\'';
    end += mysql_real_escape_string(conn2, end, argument, (strlen(argument)));
    *end++ = '\'';
    *end++ = ')';
    *end++ = '\0';
  ...
  if (CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS || CONFIG_CAMPAIGN == CAMPAIGN_FORGOTTEN_REALMS) {
    // Open mysql connection
    conn2 = mysql_init(NULL);
    /* Connect to database */
    temp = CONFIG_CAMPAIGN;
    CONFIG_CAMPAIGN = CAMPAIGN_DRAGONLANCE;
    if (!mysql_real_connect(conn2, MYSQL_SERVER, "dlmud", MYSQL_PASSWD, "dlmud", 0, NULL, 0)) {
      log("Cannot connect to mysql database in note_list_all_cats.");
    }
    CONFIG_CAMPAIGN = temp;
    char *end;
  ...
  if (CONFIG_CAMPAIGN == CAMPAIGN_DRAGONLANCE || CONFIG_CAMPAIGN == CAMPAIGN_FORGOTTEN_REALMS) {
    // Open mysql connection
    conn2 = mysql_init(NULL);
    /* Connect to database */
    temp = CONFIG_CAMPAIGN;
    CONFIG_CAMPAIGN = CAMPAIGN_STAR_WARS;
     if (!mysql_real_connect(conn2, MYSQL_SERVER, "swrpg", "kingfoil", "swrpg", 0, NULL, 0)) {
      log("Cannot connect to mysql database in note_list_all_cats.");
    }
    CONFIG_CAMPAIGN = temp;
    char *end;
  ...
  struct char_data *tch = NULL;
  for (tch = character_list; tch; tch = tch->next)
    if (tch->desc && !IS_NPC(tch))
      send_to_char(tch, "@R%s@@%s d20chats: %s@n\r\n", GET_NAME(ch), campaign_abbrevs[CONFIG_CAMPAIGN], argument);
  
  return;
}
void send_d20_chat_to_all(void) {
  MYSQL *conn2 = NULL;
  // Open mysql connection
  conn2 = mysql_init(NULL);
  /* Connect to database */
  ...
  struct char_data *tch = NULL;
  sprintf(query, "SELECT name, theme, message FROM d20chats");
  mysql_query(conn2, query);
  ...
  sprintf(query, "DELETE FROM d20chats WHERE 1");
  mysql_query(conn2, query);
  mysql_close(conn2);
}
