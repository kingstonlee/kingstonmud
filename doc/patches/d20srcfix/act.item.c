...
ACMD(do_harvest_new)
{
  ...
  char buf[200];
  sprintf(buf, "You begin to harvest %s.", node->short_description);
  act(buf, FALSE, ch, 0, NULL, TO_CHAR);

  // Tell the room the character made something. 
  sprintf(buf, "$n begins to harvest %s.", node->short_description);
...
int hands(struct char_data * ch)
{
  ...
  if (GET_EQ(ch, WEAR_WIELD1)) {
    if ((GET_EQ(ch, WEAR_WIELD) && IS_SET(weapon_list[GET_OBJ_VAL(GET_EQ(ch, WEAR_WIELD), 0)].weaponFlags, WEAPON_FLAG_DOUBLE)) ||
        ...
        ((OBJ_FLAGGED(GET_EQ(ch, WEAR_WIELD1), ITEM_2H)|| IS_SET(weapon_list[GET_OBJ_VAL(GET_EQ(ch, WEAR_WIELD1), 0)].weaponFlags, WEAPON_FLAG_TWO_HANDED))
...
void perform_wear(struct char_data *ch, struct obj_data *obj, int where)
{
  ...
  int wear_bitvectors[] = {
    ...
    ITEM_WEAR_TAKE, ITEM_WEAR_TAKE, ITEM_WEAR_WAIST, ITEM_WEAR_TAKE,
    ITEM_WEAR_TAKE, ITEM_WEAR_TAKE, ITEM_WEAR_WAIST, ITEM_WEAR_TAKE,
    ITEM_WEAR_TAKE, ITEM_WEAR_TAKE, ITEM_WEAR_WAIST, ITEM_WEAR_TAKE,
    ITEM_WEAR_TAKE, ITEM_WEAR_BARDING  
  };
    ...
    for (obj = ch->carrying; obj; obj = next_obj) {
      ...
      if (CAN_SEE_OBJ(ch, obj) && (where = find_eq_pos(ch, obj, 0)) >= 0) {
        if (MAX(1, GET_CLASS_LEVEL(ch)) < GET_OBJ_LEVEL(obj)) {
          ...
          items_worn++;
        if (GET_EQ(ch, WEAR_BODY) && where != WEAR_SHIELD && where != WEAR_ABOUT && (GET_OBJ_TYPE(GET_EQ(ch, WEAR_BODY)) == ITEM_ARMOR ||
