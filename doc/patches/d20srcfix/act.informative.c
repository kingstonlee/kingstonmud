...
*/
  ...
  sprintf(buf, "@C$s primary hand holds@n %s@n.", GET_EQ(i, WEAR_WIELD1) ? GET_EQ(i, WEAR_WIELD1)->short_description : "nothing");
  act(buf, false, i, 0, ch, TO_VICT);
  if (GET_EQ(i, WEAR_WIELD2))
    sprintf(buf, "@c$s secondary hand holds@n %s@n.", GET_EQ(i, WEAR_WIELD2)->short_description);
  else if (GET_EQ(i, WEAR_SHIELD))
    sprintf(buf, "@c$s secondary hand holds@n %s@n.", GET_EQ(i, WEAR_SHIELD)->short_description);
  else
    sprintf(buf, "@c$s secondary hand holds@n nothing@n.");
  act(buf, false, i, 0, ch, TO_VICT);
  send_to_char(ch, "\r\n");

  //show floating
  send_to_char(ch, "@YFloating nearby:@n\r\n");
  if (GET_EQ(i, WEAR_ABOVE))
    send_to_char(ch, "  %s@n\r\n", GET_EQ(i, WEAR_ABOVE)->short_description);
  if (!GET_EQ(i, WEAR_ABOVE))
    send_to_char(ch, "  nothing@\r\n");
  send_to_char(ch, "\r\n");
  ...
  if (GET_EQ(i, WEAR_ONBACK_2))
    send_to_char(ch, "  %s@n\r\n", GET_EQ(i, WEAR_ONBACK_2)->short_description);
  if (GET_EQ(i, WEAR_ARMS))
    send_to_char(ch, "  %s@n\r\n", GET_EQ(i, WEAR_ARMS)->short_description);
  if (GET_EQ(i, WEAR_LEGS))
    send_to_char(ch, "  %s@n\r\n", GET_EQ(i, WEAR_LEGS)->short_description);
  ...
  if (!GET_EQ(i, WEAR_HEAD) && !GET_EQ(i, WEAR_ABOUT) && !GET_EQ(i, WEAR_BODY) && !GET_EQ(i, WEAR_ONBACK_2) && !GET_EQ(i, WEAR_ARMS) 
    && !GET_EQ(i, WEAR_LEGS) && !GET_EQ(i, WEAR_HANDS) && !GET_EQ(i, WEAR_WAIST_1) && !GET_EQ(i, WEAR_FEET))
    send_to_char(ch, "  nothing@n\r\n");
  send_to_char(ch, "\r\n");

  //show jewelry
  send_to_char(ch, "@M%s's jewelry consists of:@n\r\n", has_intro(ch, i) ? GET_NAME(i) : tmpdesc);
	free(tmpdesc);
  if (GET_EQ(i, WEAR_NECK_1))
    send_to_char(ch, "  %s@n\r\n", GET_EQ(i, WEAR_NECK_1)->short_description);
  if (GET_EQ(i, WEAR_NECK_2))
    send_to_char(ch, "  %s@n\r\n", GET_EQ(i, WEAR_NECK_2)->short_description);
  ...
  if (!GET_EQ(i, WEAR_NECK_1) && !GET_EQ(i, WEAR_NECK_2) && !GET_EQ(i, WEAR_FINGER_R) && !GET_EQ(i, WEAR_FINGER_L) && !GET_EQ(i, WEAR_WRIST_R) &&
      !GET_EQ(i, WEAR_WRIST_L))
    send_to_char(ch, "  nothing\r\n@n");
  send_to_char(ch, "\r\n");

      //show barding
  send_to_char(ch, "@yMounts barding:@n\r\n");
  if (GET_EQ(i, WEAR_BARDING))
    send_to_char(ch, "  %s@n\r\n", GET_EQ(i, WEAR_BARDING)->short_description);
  if (!GET_EQ(i, WEAR_BARDING))
    send_to_char(ch, "  nothing@\r\n");
  send_to_char(ch, "\r\n");
...
if (GET_MAX_KI(rec) > 0 && GET_CLASS_RANKS(ch, CLASS_MONK) > 0)
  ...
  send_to_char(rec, "You have @Y%d@n reputation points accumulated.\r\n", GET_QUESTPOINTS(rec));
...
((int)rec->time.played / 3600) % 24 , (int)((rec->time.played % 3600) / 60));
  ...
  send_to_char(rec, "You have @Y%d@n gold and @Y%d@n more in the bank.\r\n", GET_GOLD(ch), GET_BANK_GOLD(ch));
...
//  char penstr[80];
  ...
  send_to_char(ch, "@r=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=@n\r\n");
  send_to_char(ch, "@rClass: @y%s @rRace: @y%s @rLevel: @y%d@n\r\n", pc_class_types_fr[CONFIG_CAMPAIGN][(int)GET_CLASS(ch)], pc_race_types[(int)GET_RACE(ch)], GET_LEVEL(ch));
...
ACMD(do_who)
{
  ...
  char staff_title[200], players_title[200];
  int x = 0, y = 0;
...
//  if (GET_ADMLEVEL(ch) >= 0)
  sprintf(staff_title, "Immortals Online");
  send_to_char(ch, "\r\n");
  for (x = 0; x < ((80 - strlen(staff_title)) / 2); x++)
    send_to_char(ch, "@y-@n");
  send_to_char(ch, "%s", staff_title);
  for (y = x + strlen(staff_title); y < 80; y++)
    send_to_char(ch, "@y-@n");
  send_to_char(ch, "\r\n");
...
//        continue;
       ...
       send_to_char(ch, "@C[%-12.12s]@n ", admin_level_names[GET_ADMLEVEL(tch)]);
       send_to_char(ch, "%s%-42.42s %s", 
  ...
  send_to_char(ch, "M");
    ...
    sprintf(players_title, "Mortals Online");
    send_to_char(ch, "\r\n");
  for (x = 0; x < ((80 - strlen(players_title)) / 2); x++)
    send_to_char(ch, "@y-@n");
  send_to_char(ch, "%s", players_title);
  for (y = x + strlen(players_title); y < 80; y++)
    send_to_char(ch, "@y-@n");
  send_to_char(ch, "\r\n");
  ...
  !PLR_FLAGGED(tch, PLR_THIEF))
    ...
    num_can_see++;
    char *tmpdesc = NULL;
       send_to_char(ch, "@C[%-12.12s]@n @Y[%-2.2d]@n @G[%-7.7s]@n %s%-42.42s %s", has_intro(ch, tch) ? (GET_CLAN(tch) ? get_blank_clan_whostring(GET_CLAN(tch)) : "UnClanned") : "Not Known", 
                    GET_CLASS_LEVEL(tch), race_list[GET_REAL_RACE(tch)].abbrev,
...
//                 (tmpdesc = which_desc(tch)));
  ...
  if (num_can_see == 0)
    ...
    send_to_char(ch, "%d characters displayed.  ", num_can_see);
  send_to_char(ch, "%d %s in game in total (not necessarily visible)\r\n", tot_players, tot_players == 1 ? "person" : "people");
  send_to_char(ch, "\r\n");
  send_to_char(ch, "P = PVP, L = LEVELLING, D = SETTING DESCS, I = INVIS, M = MAILING, C = CRAFTING,\r\n"
                   "W = WRITING, A = AFK, F = FIGHTING, G = LOOKING FOR GROUP\r\n");
}
...
ACMD(do_affstats) {
  ...
  send_to_char(k, "@y%-25s %-5s %-11s\r\n------------------------- ----- -----------\r\n@n",
               "Affect Name", "Level", "Duration");
  ...
  if (k->affected) {
    for (aff = k->affected; aff; aff = aff->next) {
      ...
      send_to_char(k, "@w%-25s %-5d %-11s\r\n@n", skill_name(aff->type), aff->level, duration);
    }
  }
  ...
  if (k->affectedv) {
    for (aff = k->affectedv; aff; aff = aff->next) {
      ...
      /*sprintf(modifier, "%+d to %s", aff->modifier, apply_text[(int) aff->location]);
      sprintf(bitvector, "sets %s", affected_bits[aff->bitvector]);*/
      send_to_char(k, "@w%-25s %-11s\r\n@n", skill_name(aff->type), duration);
    }
  }
...
ACMD(do_post_forums)
{
  send_to_char(ch, "All bugs, ideas and typos should be posted on the forums at http://www.skmud.com/ .\r\n");
}
...
ACMD(do_d20who) {
  ...
  if (!*arg) {
    send_to_char(ch, "Do you want to see who lists for:\r\n");
    for (i = 0; i < NUM_CAMPAIGNS; i++)
      send_to_char(ch, "-- %s\r\n", campaign_abbrevs[i]);
    return;
  }

  int temp = CONFIG_CAMPAIGN;

  if (is_abbrev(CAP(arg), campaign_abbrevs[CAMPAIGN_STAR_WARS])) {
    CONFIG_CAMPAIGN = CAMPAIGN_STAR_WARS;
    send_to_char(ch, "People online on d20MUD: %s\r\n\r\n", campaign_names[CONFIG_CAMPAIGN]);
  }
  else if (is_abbrev(CAP(arg), campaign_abbrevs[CAMPAIGN_DRAGONLANCE])) {
    CONFIG_CAMPAIGN = CAMPAIGN_DRAGONLANCE;
    send_to_char(ch, "People online on d20MUD: %s\r\n\r\n", campaign_names[CONFIG_CAMPAIGN]);
  }
  else if (is_abbrev(CAP(arg), campaign_abbrevs[CAMPAIGN_FORGOTTEN_REALMS])) {
  ...
  conn = mysql_init(NULL);
  /* Connect to database */
  if (!mysql_real_connect(conn, MYSQL_SERVER, (CONFIG_CAMPAIGN==CAMPAIGN_STAR_WARS)?"swrpg":((CONFIG_CAMPAIGN == CAMPAIGN_DRAGONLANCE)?"dlmud":"frmud"), (CONFIG_CAMPAIGN==CAMPAIGN_STAR_WARS)?"kingfoil":MYSQL_PASSWD, (CONFIG_CAMPAIGN==CAMPAIGN_STAR_WARS)?"swrpg":((CONFIG_CAMPAIGN == CAMPAIGN_DRAGONLANCE)?"dlmud":"frmud"), 0, NULL, 0)) {
    log("Cannot connect to mysql database in gather info.");
  }

  sprintf(query, "SELECT title, level, clan_num, race FROM player_data WHERE online='1'");
   mysql_query(conn, query);
  res = mysql_use_result(conn);
  if (res != NULL) {
    while ((row = mysql_fetch_row(res)) != NULL) {
      send_to_char(ch, "@C[%-12.12s]@n @Y[%-2.2d]@n @G[%-15.15s]@n %-42.42s\r\n", (atoi(row[2]) ? get_blank_clan_whostring(atoi(row[2])) : "UnClanned"), atoi(row[1]), row[3], row[0]);
    }
  }

  CONFIG_CAMPAIGN = temp;
  mysql_free_result(res);
  mysql_close(conn);
