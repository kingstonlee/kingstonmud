...
char *shuttle_locales_dl[][7] = {
  ...
  {"Taman Busuk",                    "16822",   "500",      "Dragonarmies",     "home of the dragonarmies & knights of takhisis", "380", "140"},
  {"Abanasinia",                     "16811", "500",      "Free People of Ansalon",  "home of the qualinesti, tarsis, solace and plainsmen tribes", "460", "160"},
...
ACMD(do_speeder) {
  ...
  if (subcmd == SCMD_SPEEDER) {
    send_to_char(ch, "%s aren't implemented yet.  You'll have to use the %s command for now.\r\n", (CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS) ? "Speeders" : "Caravans", (CONFIG_CAMPAIGN == CAMPAIGN_STAR_WARS) ? "taxi" : "carriage");
    return;
  }
